...
1. **GitHub Actions workflows** — search `.github/workflows/*.yml` for `amannn/action-semantic-pull-request`. If found and a `scopes:` list is present in the action's `with:` block, extract it (newline- or comma-separated) and treat scopes as restricted — this is a hit. If the action is found but has no `scopes:` block, treat scopes as unrestricted and stop — this is still a hit (CI is not enforcing scopes).
...
Note the discovered allowlist (or `unrestricted`) and its source in working memory for use in step 5b and later recording in the state file as part of Step 6. Do not write to the state file here — Step 6 is the single write point for the Phase 2 block.
...
**Scope Assumption:** <only if autonomous mode picked a scope because no allowed scope cleanly matched the diff; else omit this line>
**Commits:**
- `<hash>`: <commit message>
**Files Changed:**
